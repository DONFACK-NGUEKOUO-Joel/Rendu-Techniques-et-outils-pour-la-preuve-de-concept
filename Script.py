...
class Node:
    def __init__(self, offset, field_type, length):
        self.offset = offset
...
class StreamPacket:
    ...
    def __init__(self, raw_data: bytes, original_packet=None):
        ...
        self.current_offset = 0           # Le pointeur dans la charge utile
...
def load_and_preprocess_pcap(pcap_file_path: str, protocol_layer: str = 'Raw') -> List[List[StreamPacket]]:
   ...
   # === Ajout indispensable : définition correcte de StreamPacket ===
class StreamPacket:
    def __init__(self, raw_data, original_packet):
        self.raw_data = raw_data
        self.original_packet = original_packet


def load_and_preprocess_pcap(filename, protocol_layer):
